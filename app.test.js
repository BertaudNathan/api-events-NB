...
beforeAll(async () => {
    await initDB();
    await pool.query('DELETE FROM events'); // Table vide avant les tests
});
...
describe('API Events', () => {
    it('should create a new event', async () => {
        ...
        const response = await request(app)
            .post('/events')
            .send({ title: 'Test Event', date: today.toISOString().split('T')[0], categorie: 'Music', participants: 10, lieu: 'Paris' });
